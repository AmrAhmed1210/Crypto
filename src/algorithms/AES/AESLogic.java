package algorithms.AES;

import java.util.Base64;

public class AESLogic {

    private static final int[] SBOX = {
            
    };

    private static final int[] INV_SBOX = {
            
    };

    private static final int[] RCON = {
            0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36,
            0x6c,0xd8,0xab,0x4d,0x9a,0x2f,0x5e,0xbc,0x63,0xc6,
            0xaf,0x5c,0xb8,0x71
    };

    // -------------------------------------------------------------------------
    // Public API
    // -------------------------------------------------------------------------

    public static String encrypt(String plaintext, String key) throws Exception {
        byte[] padded     = pkcs5Pad(plaintext.getBytes("UTF-8"));
        int[][] roundKeys = keyExpansion(key.getBytes("UTF-8"));
        byte[] output     = new byte[padded.length];
        for (int i = 0; i < padded.length; i += 16) {
            byte[] block = new byte[16];
            System.arraycopy(padded, i, block, 0, 16);
            byte[] enc = encryptBlock(block, roundKeys);
            System.arraycopy(enc, 0, output, i, 16);
        }
        return Base64.getEncoder().encodeToString(output);
    }

    public static String decrypt(String ciphertext, String key) throws Exception {
        byte[] cipher     = Base64.getDecoder().decode(ciphertext);
        int[][] roundKeys = keyExpansion(key.getBytes("UTF-8"));
        byte[] output     = new byte[cipher.length];
        for (int i = 0; i < cipher.length; i += 16) {
            byte[] block = new byte[16];
            System.arraycopy(cipher, i, block, 0, 16);
            byte[] dec = decryptBlock(block, roundKeys);
            System.arraycopy(dec, 0, output, i, 16);
        }
        return new String(pkcs5Unpad(output), "UTF-8");
    }

    public static String validateKey(String key) {
        if (key == null || key.isEmpty()) return "Key cannot be empty!";
        int len = key.length();
        if (len != 16 && len != 24 && len != 32)
            return "Key must be 16, 24, or 32 characters (got " + len + ")";
        return null;
    }

    public static String keyLabel(String key) {
        return "AES-" + (key.length() * 8);
    }

    // -------------------------------------------------------------------------
    // Block encryption / decryption
    // -------------------------------------------------------------------------

    private static byte[] encryptBlock(byte[] block, int[][] rk) {
        int nr    = rk.length - 1;
        int[][] s = toState(block);
        addRoundKey(s, rk[0]);
        for (int r = 1; r < nr; r++) {
            subBytes(s);
            shiftRows(s);
            mixColumns(s);
            addRoundKey(s, rk[r]);
        }
        subBytes(s);
        shiftRows(s);
        addRoundKey(s, rk[nr]);
        return fromState(s);
    }

    private static byte[] decryptBlock(byte[] block, int[][] rk) {
        int nr    = rk.length - 1;
        int[][] s = toState(block);
        addRoundKey(s, rk[nr]);
        for (int r = nr - 1; r >= 1; r--) {
            invShiftRows(s);
            invSubBytes(s);
            addRoundKey(s, rk[r]);
            invMixColumns(s);
        }
        invShiftRows(s);
        invSubBytes(s);
        addRoundKey(s, rk[0]);
        return fromState(s);
    }

    // -------------------------------------------------------------------------
    // AES transformations
    // -------------------------------------------------------------------------

    private static void subBytes(int[][] s) {
        for (int r = 0; r < 4; r++)
            for (int c = 0; c < 4; c++)
                s[r][c] = SBOX[s[r][c] & 0xFF];
    }

    private static void invSubBytes(int[][] s) {
        for (int r = 0; r < 4; r++)
            for (int c = 0; c < 4; c++)
                s[r][c] = INV_SBOX[s[r][c] & 0xFF];
    }

    private static void shiftRows(int[][] s) {
        int t;
        t = s[1][0]; s[1][0] = s[1][1]; s[1][1] = s[1][2]; s[1][2] = s[1][3]; s[1][3] = t;
        t = s[2][0]; int t2 = s[2][1]; s[2][0] = s[2][2]; s[2][1] = s[2][3]; s[2][2] = t; s[2][3] = t2;
        t = s[3][3]; s[3][3] = s[3][2]; s[3][2] = s[3][1]; s[3][1] = s[3][0]; s[3][0] = t;
    }

    private static void invShiftRows(int[][] s) {
        int t;
        t = s[1][3]; s[1][3] = s[1][2]; s[1][2] = s[1][1]; s[1][1] = s[1][0]; s[1][0] = t;
        t = s[2][0]; int t2 = s[2][1]; s[2][0] = s[2][2]; s[2][1] = s[2][3]; s[2][2] = t; s[2][3] = t2;
        t = s[3][0]; s[3][0] = s[3][1]; s[3][1] = s[3][2]; s[3][2] = s[3][3]; s[3][3] = t;
    }

    private static void mixColumns(int[][] s) {
        for (int c = 0; c < 4; c++) {
            int s0 = s[0][c], s1 = s[1][c], s2 = s[2][c], s3 = s[3][c];
            s[0][c] = gfMul(2, s0) ^ gfMul(3, s1) ^ s2           ^ s3;
            s[1][c] = s0           ^ gfMul(2, s1) ^ gfMul(3, s2) ^ s3;
            s[2][c] = s0           ^ s1            ^ gfMul(2, s2) ^ gfMul(3, s3);
            s[3][c] = gfMul(3, s0) ^ s1            ^ s2           ^ gfMul(2, s3);
        }
    }

    private static void invMixColumns(int[][] s) {
        for (int c = 0; c < 4; c++) {
            int s0 = s[0][c], s1 = s[1][c], s2 = s[2][c], s3 = s[3][c];
            s[0][c] = gfMul(0x0e, s0) ^ gfMul(0x0b, s1) ^ gfMul(0x0d, s2) ^ gfMul(0x09, s3);
            s[1][c] = gfMul(0x09, s0) ^ gfMul(0x0e, s1) ^ gfMul(0x0b, s2) ^ gfMul(0x0d, s3);
            s[2][c] = gfMul(0x0d, s0) ^ gfMul(0x09, s1) ^ gfMul(0x0e, s2) ^ gfMul(0x0b, s3);
            s[3][c] = gfMul(0x0b, s0) ^ gfMul(0x0d, s1) ^ gfMul(0x09, s2) ^ gfMul(0x0e, s3);
        }
    }

    private static void addRoundKey(int[][] s, int[] rk) {
        for (int c = 0; c < 4; c++) {
            s[0][c] ^= (rk[c] >>> 24) & 0xFF;
            s[1][c] ^= (rk[c] >>> 16) & 0xFF;
            s[2][c] ^= (rk[c] >>>  8) & 0xFF;
            s[3][c] ^=  rk[c]          & 0xFF;
        }
    }

    // -------------------------------------------------------------------------
    // Key expansion
    // -------------------------------------------------------------------------

    private static int[][] keyExpansion(byte[] key) {
        int nk    = key.length / 4;
        int nr    = nk + 6;
        int total = 4 * (nr + 1);
        int[] w   = new int[total];

        for (int i = 0; i < nk; i++)
            w[i] = ((key[4*i] & 0xFF) << 24) | ((key[4*i+1] & 0xFF) << 16)
                    | ((key[4*i+2] & 0xFF) << 8) |  (key[4*i+3] & 0xFF);

        for (int i = nk; i < total; i++) {
            int t = w[i - 1];
            if (i % nk == 0)
                t = subWord(rotWord(t)) ^ (RCON[i/nk - 1] << 24);
            else if (nk > 6 && i % nk == 4)
                t = subWord(t);
            w[i] = w[i - nk] ^ t;
        }

        int[][] rk = new int[nr + 1][4];
        for (int r = 0; r <= nr; r++)
            for (int c = 0; c < 4; c++)
                rk[r][c] = w[4*r + c];
        return rk;
    }

    private static int rotWord(int w) { return (w << 8) | ((w >>> 24) & 0xFF); }

    private static int subWord(int w) {
        return (SBOX[(w >>> 24) & 0xFF] << 24) | (SBOX[(w >>> 16) & 0xFF] << 16)
                | (SBOX[(w >>>  8) & 0xFF] <<  8) |  SBOX[w & 0xFF];
    }

    // -------------------------------------------------------------------------
    // GF(2^8) multiplication
    // -------------------------------------------------------------------------

    private static int gfMul(int a, int b) {
        int r = 0;
        b &= 0xFF;
        while (a > 0) {
            if ((a & 1) != 0) r ^= b;
            b <<= 1;
            if ((b & 0x100) != 0) b ^= 0x1b;
            b &= 0xFF;
            a >>= 1;
        }
        return r;
    }

    // -------------------------------------------------------------------------
    // PKCS5 padding
    // -------------------------------------------------------------------------

    private static byte[] pkcs5Pad(byte[] data) {
        int pad  = 16 - (data.length % 16);
        byte[] out = new byte[data.length + pad];
        System.arraycopy(data, 0, out, 0, data.length);
        for (int i = data.length; i < out.length; i++) out[i] = (byte) pad;
        return out;
    }

    private static byte[] pkcs5Unpad(byte[] data) throws Exception {
        int pad = data[data.length - 1] & 0xFF;
        if (pad < 1 || pad > 16) throw new Exception("Invalid padding");
        for (int i = data.length - pad; i < data.length; i++)
            if ((data[i] & 0xFF) != pad) throw new Exception("Corrupt padding");
        byte[] out = new byte[data.length - pad];
        System.arraycopy(data, 0, out, 0, out.length);
        return out;
    }

    // -------------------------------------------------------------------------
    // State matrix helpers
    // -------------------------------------------------------------------------

    private static int[][] toState(byte[] b) {
        int[][] s = new int[4][4];
        for (int c = 0; c < 4; c++)
            for (int r = 0; r < 4; r++)
                s[r][c] = b[c*4 + r] & 0xFF;
        return s;
    }

    private static byte[] fromState(int[][] s) {
        byte[] b = new byte[16];
        for (int c = 0; c < 4; c++)
            for (int r = 0; r < 4; r++)
                b[c*4 + r] = (byte) s[r][c];
        return b;
    }
}